...
local_pos_pub = rospy.Publisher('/A/mavros/setpoint_position/local', PoseStamped, queue_size=10)
state_sub = rospy.Subscriber('/A/mavros/state', State, state_cb)
Cur_position_sub = rospy.Subscriber('/A/mavros/local_position/odom', Odometry, curpos_cb)
arming_client = rospy.ServiceProxy('/A/mavros/cmd/arming', CommandBool)
set_mode_client = rospy.ServiceProxy('/A/mavros/set_mode', SetMode) 
uav1_waypoint_sub = rospy.Subscriber('agent3_path',agent3_path,update_waypoints)
uav1_update_pub = rospy.Publisher('uav1',uav1,queue_size=10)
...
def position_control_uav1():
    index = 0
    global way_points
    rospy.init_node('UAV1', anonymous=True)
